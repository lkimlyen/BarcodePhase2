...
public class WarehousingWDPresenter implements WarehousingWDContract.Presenter {
    ...
    public void checkBarcode(long orderId, String barcode) {
        ...
        ProductWarehouseEntity model = ListProductWarehouseManager.getInstance().getProductByBarcode(barcodeSplit);
        if (model != null) {
            String finalBarcode = barcode;
            localRepository.getProductWarehouse(model).subscribe(new Action1<ProductWarehouseModel>() {
                @Override
